...
```mermaid
flowchart LR
    A["本机 Codex App Server"] -->|"account/rateLimits/read"| B["CodexUsageMonitor"]
    A2["Codex rollout JSONL"] -->|"兼容后备：只读尾部"| B
...
- 后备适配器单文件最多读取尾部 4 MiB，分别解析可选的 `payload.rate_limits.primary` 与 `secondary`，再按 `window_minutes` 去重、排序；不把字段名硬编码为 5 小时或 7 天。
- 任何坏行、文件轮换或单个会话损坏都不会阻断其他候选文件。
- 旧版 App Server 或 rollout 后备数据不包含可用重置次数时，界面明确显示 `—`；两种来源都不可用时返回明确标记的 `demo` 快照。

App Server 与本地会话格式都被隔离在适配器和契约测试之后；上游字段变化时不需要改 UI。
...
| 层 | 能力 | 明确禁止 |
| --- | --- | --- |
| Electron 主进程 | 调用本机 Codex 只读额度接口、读取后备额度事件、监听文件变化、管理窗口与托盘、按用户设置查询天气 | 读取认证文件、消耗重置次数、向天气服务发送 Codex 数据 |
